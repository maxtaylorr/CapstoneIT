...
extension BaseNC: UINavigationControllerDelegate {
    ...
    private func pushOperationTransitionAnimator(for vc: UIViewController) ->
        UIViewControllerAnimatedTransitioning? {
            let controller = vc as? SideView
            guard let style = transitionStyle else { return nil }
            switch style {
            case .sideBySide: return SideBySidePushTransitionAnimator(direction: controller?.directionToRoot ?? .left, navigationControl: self)
            }
    }
...
extension BaseNC {
    func addUniversalBackground() {
        ...
        let img = #imageLiteral(resourceName: "Bar Image")
